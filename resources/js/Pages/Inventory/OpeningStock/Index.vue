...
import {success,error,currency,} from '@/Utils'
import {formatDate,} from '@/Utils'
import OpeningStockForm from './Partials/OpeningStockForm.vue'
import OpeningStockViewModal    from './Partials/OpeningStockViewModal.vue'
...
const pageTitle = computed(() => 'Opening Stock')
...
const isAllSelected = computed(() => {
    ...
    const totalRows =
        openingStocks.value?.data?.length ?? 0
...
function editOpeningStock(item)
{
    if (item.status === 'Posted') {

        error(
            'Opening stock that has been posted cannot be edited.'
        )
...
function showOpeningStock(item)
{
    ...
    router.post(
        ...
        {},
        {
            ...
            onError: () => {
                ...
                error(
                    'Failed to duplicate opening stock.'
                )
...
function openDelete(item)
{
    ...
    if (item.status !== 'Draft') {
        ...
        error(
            'Only Draft opening stock can be deleted.'
        )
...
const bulkDeleteMessage = computed(() => {
    ...
    return `Are you sure you want to delete ${total} selected Opening Stock document(s)?`
...
{
    router.delete(
        route('opening-stocks.bulk-delete'),
        {
            data: {
...
function openPost(openingStock)
{
    postItem.value = openingStock
...
function openReject(openingStock)
{
    rejectItem.value = openingStock
    ...
    router.post(
...

        {
            ...
            onError: (errors) => {
                     ...
                     error(
                    'Failed to reject Opening Stock.'
                )
...
const postMessage = computed(() => {
    ...
    return `Are you sure you want to post "${postItem.value.number}"? Once posted, inventory stock will be updated.`
})
/*
|--------------------------------------------------------------------------
| Lifecycle
...
async function openView(item)
{
    ...
    try {
        ...
        error(
            'Failed to load opening stock detail.'
        )
    ...
    >
        ...
        <div class="space-y-6">
            ...
            <PageHeader
                icon="📦"
                title="Opening Stock"
                subtitle="Manage opening stock transactions."
            />
            ...
            <div class="mt-6">
                ...
                >
                    ...
                    <DataTableBody>
                        ...
                        <DataTableRow
                            v-for="item in openingStocks.data"
                            :key="item.id"
                        >
                ...
                <TableEmpty
                    v-else
                    icon="📦"
                    title="No Opening Stock Found"
                    description="There are no opening stock transactions available."
                >
            ...
            <PageHeader
                ...
                :subtitle="
                    formMode === 'create'
                        ...
                        : 'Update opening stock transaction.'
                "
            />
                ...
                <template v-if="postItem">
                    ...
                    >
                        ...
                        <p
                            ...
                            "
                        >
                            Please make sure the quantity, unit,
                            and unit cost are correct.
                            Posting this transaction will update
                            the inventory stock and create an
                            inventory movement.
                        </p>
